        ...
        function updateUsername(newUsername) {
            return new Promise(function(resolve, reject) { 
                ...
                if (newUsername && newUsername !== newUserData.username) {
                    client.hexists('usersByUsername', newUsername, function(err, resp) {
                        if (err || resp) {
                            reject(err);
                            return;
                        }
                        ...
                        client.hdel('usersByUsername', newUsername);
                        client.hset('usersByUsername', newUsername, newUserData.id);
                    });
                }
                resolve(newUserData);
            });
...
function getUserFromUsername(client, username, callback) {
    getUserIDFromUsername(client, username, function(err, id) {
        if (err && err !== 'no_such_user') {
            callback(err);
        } else if (!id) {
            callback('no_such_user');
...
function getUserIDFromUsername(client, username, callback) {
    client.hget('usersByUsername', username, function(err, resp) {
        if (err) {
            callback('db_error');
        } else if (!resp) {
            callback('no_such_user');
